# Verify PSD via Data Import front slot: uses IDSTACK_AUTO_DESIGN env hook.
...
$env:IDSTACK_AUTO_DESIGN = "D:\ID-Stack\tools\sample-data\demopsd.psd"
...
foreach ($i in $items) { if ($i.Current.Name -eq "Data Import") { Click-El $i; break } }
Start-Sleep -Seconds 2

# Invoke Browse Front Design (auto hook loads the PSD)
$btn = $root.FindFirst([System.Windows.Automation.TreeScope]::Descendants,
  (New-Object System.Windows.Automation.AndCondition(
    ...
    (New-Object System.Windows.Automation.PropertyCondition(
      [System.Windows.Automation.AutomationElement]::NameProperty, "Browse Front Design")))))
if (-not $btn) { Write-Host "BUTTON NOT FOUND"; exit 1 }
try { ($btn.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern)).Invoke(); Write-Host "Invoked Browse Front Design" }
catch { Click-El $btn; Write-Host "Clicked (mouse)" }
...
foreach ($t in $texts) {
  $n = $t.Current.Name
  if ($n -match "placeholder|PSD|layers|Imported|design|Could") { Write-Host ("TXT: " + $n) }
}
...
$g.Dispose()
$bmp.Save("D:\ID-Stack\tools\ui-test\d9-psd-dataimport.png", [System.Drawing.Imaging.ImageFormat]::Png)
$bmp.Dispose()
Write-Host "saved d9-psd-dataimport.png"
